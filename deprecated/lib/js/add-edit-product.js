jQuery( document ).ready( function($) {
	...
	*/
	...
	// Resend Email
	$('#it-exchange-invoicing-cancel-auto-invoicing').on('click', function(event){
		event.preventDefault();
		var data = {
			action: 'it-exchange-invoicing-cancel-auto-invoicing',
			invoiceID: $('#it-exchange-invoicing-cancel-auto-invoicing').data('invoice-id')
		};

		$.post(ajaxurl, data, function(response) {
			$('.it-exchange-invoice-field-container-parent-invoice-list').html( response );
		});
	});
	...
	$( '.invoice-section' ).on( 'click', '#invoice-field-container-recurring-options-enabled', function( event ) {
		$( '#recurring-options-enabled-div' ).toggleClass( 'hidden' );
	});
});
